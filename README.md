...
**本仓库为整站完整源码**：95 款在线小工具的页面 + 零依赖 Node 服务（静态托管与开放 API 同进程），本地 `node server.js` 即可一键自建整套站点。

95 款在线小工具：图片处理、文本转换、加密编码、日期计算、文本生成、生活查询等。所有工具均在浏览器本地完成，不上传用户数据。
...
│   └── lib/            # 第三方库（qrcode 等）
├── tools/              # 95 个工具页（一工具一页面）
└── data/               # API 限流计数（服务启动时自动生成）
```
